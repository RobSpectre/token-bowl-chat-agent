...
class TokenBowlAgent:
    ...
    def _on_error(self, error: Exception) -> None:
        ...
        self.stats.errors += 1
        logger.error("WebSocket error: %s", error_msg, exc_info=error)
    ...
    async def _process_message_batch(self, messages: list[MessageQueueItem]) -> None:
        ...
        try:
            ...
            try:
                ...
                error_msg = str(e)
                logger.error(
                    "Error sending response: %s (message_count=%d)",
                    error_msg,
                    len(new_messages),
                    exc_info=True,
                )
            ...
            error_msg = str(e)
            logger.error(
                "Error processing messages: %s (message_count=%d, messages=%s)",
                error_msg,
                len(new_messages),
                [
                    {
        ...
        while self.is_running:
            try:
                ...
                if time_since_last_flush >= self.queue_interval and self.message_queue:
                    async with self.processing_lock:
                        # Check if we're in cooldown BEFORE clearing the queue
                ...
                self.stats.errors += 1
                logger.error("Error in processor loop: %s", str(e), exc_info=True)
                if self.verbose:
        ...
        while self.is_running:
            try:
                ...
                if self.is_running:
                    self.stats.errors += 1
                    logger.error(
                        "WebSocket loop error: %s. Reconnecting...",
                        str(e),
                        exc_info=True,
                    )
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            logger.critical("Fatal error in agent run loop: %s", str(e), exc_info=True)
